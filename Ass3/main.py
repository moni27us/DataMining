...
def create_feature_matrix(meal_date_df,CGMData,feature_matrix):   
    ...
    for i in range(meal_date_df.shape[0]):
        ...
        arr=CGMData[(CGMData['DateTime']>=(meal_date-pd.to_timedelta(30, unit='m'))) & (CGMData['DateTime']<(meal_date+pd.to_timedelta(2, unit='h')))]["Sensor Glucose (mg/dL)"].values.tolist()
        if(len(arr)<24):
            continue;
        arr.reverse()  
...
def find_nearest_cluster(cluster_index,dt):
    dist_arr=[]
    min_cluster_id=0
    min_cluster_dist= 999999999 #math.inf
...
if __name__ == '__main__':
    ...
    #applying K-means
    kmeans = KMeans(n_clusters=int(num_of_clusters),random_state=0)
    ...
    answer_df.to_csv('Results.csv', index=False, header=False)